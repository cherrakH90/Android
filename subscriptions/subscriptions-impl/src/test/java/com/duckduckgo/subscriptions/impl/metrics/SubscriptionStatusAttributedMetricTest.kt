...
class SubscriptionStatusAttributedMetricTest {
    ...
    @Before
    fun setup() = runTest {
        subscriptionToggle.subscriptionRetention().setRawStoredState(State(true))
        subscriptionToggle.canEmitSubscriptionRetention().setRawStoredState(State(true))
        whenever(attributedMetricConfig.metricsToggles()).thenReturn(
            listOf(subscriptionToggle.subscriptionRetention(), subscriptionToggle.canEmitSubscriptionRetention()),
        )
        ...
        testee = SubscriptionStatusAttributedMetric(
            ...
            subscriptionsDataStore = subscriptionsDataStore,
    ...
    fun whenOnCreateAndFFDisabledThenDoNotEmitMetric() = runTest {
        subscriptionToggle.subscriptionRetention().setRawStoredState(State(false))
        whenever(attributedMetricConfig.metricsToggles()).thenReturn(
            listOf(subscriptionToggle.subscriptionRetention(), subscriptionToggle.canEmitSubscriptionRetention()),
        )
        givenSubscriptionActive()
    ...
    fun whenOnCreateAndEmitDisabledThenDoNotEmitMetric() = runTest {
        subscriptionToggle.subscriptionRetention().setRawStoredState(State(true))
        subscriptionToggle.canEmitSubscriptionRetention().setRawStoredState(State(false))
        whenever(attributedMetricConfig.metricsToggles()).thenReturn(
            listOf(subscriptionToggle.subscriptionRetention(), subscriptionToggle.canEmitSubscriptionRetention()),
        )
        givenSubscriptionActive()
    ...
    @Test
    fun whenOnCreateAndAutoRenewableSubscriptionThenEmitMetric() = runTest {
    ...
    fun whenOnCreateAndNonAutoRenewableSubscriptionThenEmitMetric() = runTest {
        whenever(authRepository.getStatus()).thenReturn(SubscriptionStatus.NOT_AUTO_RENEWABLE)
    ...
    fun whenGetMetricParametersAndOnTrialThenReturnBucketZero() = runTest {
        ...
        whenever(subscriptionsDataStore.freeTrialActive).thenReturn(true)
    ...
    fun whenGetMetricParametersThenReturnCorrectBucketValue() = runTest {
        whenever(subscriptionsDataStore.freeTrialActive).thenReturn(false)
    ...
    private fun givenSubscriptionActive() = runTest {
        whenever(authRepository.getStatus()).thenReturn(SubscriptionStatus.AUTO_RENEWABLE)
    }

    private fun givenDaysSinceSubscribed(days: Int) {
        val etZone = ZoneId.of("America/New_York")
        val now = Instant.now()
        val nowInEt = now.atZone(etZone)
        val startedInEt = nowInEt.minusDays(days.toLong())
        whenever(subscriptionsDataStore.startedAt).thenReturn(startedInEt.toInstant().toEpochMilli())
    }
}
